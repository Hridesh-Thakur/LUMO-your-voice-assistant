...
const commandDB = [
  ...
  // ---- Play songs directly on YouTube ----
  {
    ...
    action: async (msg) => {
      ...
      if (!songName) return "Tell me the song name, like 'play shape of you'.";

      // No API key set up yet — fall back to opening YouTube search results.
      if (!YOUTUBE_API_KEY || YOUTUBE_API_KEY === 'YOUR_YOUTUBE_API_KEY_HERE') {
        window.open(`https://www.youtube.com/results?search_query=${encodeURIComponent(songName)}`, '_blank');
        return `I don't have a YouTube API key set up yet, so I opened search results for ${songName} instead.`;
      }

      try {
        const res = await fetch(
          `https://www.googleapis.com/youtube/v3/search?part=snippet&type=video&maxResults=1&q=${encodeURIComponent(songName)}&key=${YOUTUBE_API_KEY}`
        );
        ...
        console.warn('LUMO: YouTube search failed', e);
        window.open(`https://www.youtube.com/results?search_query=${encodeURIComponent(songName)}`, '_blank');
  ...
  { keywords: ['open excel', 'open ms excel', 'open microsoft excel'],
    action: () => { window.open('https://excel.new', '_blank'); return "Opening a new Excel workbook..."; } },
  { keywords: ['open powerpoint', 'open ms powerpoint', 'open microsoft powerpoint'],
    action: () => { window.open('https://powerpoint.new', '_blank'); return "Opening a new PowerPoint presentation..."; } },
  { keywords: ['open word', 'open ms word', 'open microsoft word'],
    action: () => { window.open('https://word.new', '_blank'); return "Opening a new Word document..."; } },
  { keywords: ['open outlook desktop', 'open ms outlook'],
    action: () => { window.open('https://outlook.office.com', '_blank'); return "Opening Outlook..."; } },
  { keywords: ['open onenote'],
    action: () => { window.open('https://www.onenote.com/notebooks', '_blank'); return "Opening OneNote..."; } },
...
async function takeCommand(message) {
  const replyOrPromise = getReplyFor(message);
  const reply = (replyOrPromise instanceof Promise) ? await replyOrPromise : replyOrPromise;
  speak(reply);
...
const WAKE_WORD = /\b(lumo|luno|lu|you know|logo|mr ai ?mo|l ?u ?m ?o)\b/i;
